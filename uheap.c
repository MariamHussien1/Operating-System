...
void initialize_dyn_block_system()
{
	...
	int size_Of_Heap;
	size_Of_Heap = (sizeof(struct MemBlock)) * NUM_OF_UHEAP_PAGES;
	...
	sys_allocate_chunk(USER_DYN_BLKS_ARRAY , size_Of_Heap ,PERM_USER | PERM_WRITEABLE | PERM_PRESENT);
	...
	New_MemBlock->size = USER_HEAP_MAX - USER_HEAP_START;
	New_MemBlock->sva  = USER_HEAP_START;
...
void free(void* virtual_address)
{
	...
	if(block != NULL){
		sys_free_user_mem(Va, block->size);
	...
	if(size >= USER_HEAP_MAX - USER_HEAP_START){
		...
		if(ff == 1){
			...
			if(block != NULL){
				...
				int result = sys_createSharedObject(sharedVarName, s, isWritable, (void*)block->sva);
...
void* sget(int32 ownerEnvID, char *sharedVarName)
{
	...
	int size_of_shared_var = sys_getSizeOfSharedObject(ownerEnvID, sharedVarName);
	size_of_shared_var = ROUNDUP(size_of_shared_var , PAGE_SIZE);
	if(size_of_shared_var == E_SHARED_MEM_NOT_EXISTS){
		return NULL;
	}
	else if(size_of_shared_var >= USER_HEAP_MAX - USER_HEAP_START){
		return NULL;
	}
	else{
		int ff = sys_isUHeapPlacementStrategyFIRSTFIT();
		if(ff == 1){
			struct MemBlock * ptr_shared_var = NULL;
			ptr_shared_var = alloc_block_FF(size_of_shared_var);
			if (ptr_shared_var != NULL){
				insert_sorted_allocList(ptr_shared_var);
				int retval = sys_getSharedObject(ownerEnvID , sharedVarName ,(void *) ptr_shared_var->sva);
				if(retval == E_SHARED_MEM_NOT_EXISTS){
					return NULL;
				}
				else{
					return (void*)ptr_shared_var->sva;
				}
			}
			else{
				return NULL;
