...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	struct Share *shared_var;
	uint32 startaddr = ROUNDDOWN((uint32)virtual_address, PAGE_SIZE);
	uint32 end = ROUNDUP((uint32)virtual_address + size, PAGE_SIZE);
	int j;

	int m = get_share_object_ID(ownerID, shareName);
	if(m == E_SHARED_MEM_NOT_EXISTS){
		int ret = allocate_share_object(&shared_var);
		if(ret == E_NO_SHARE){
			return E_NO_SHARE;
		}
		else{
			j = 0;
			for(uint32 i = startaddr; i < end ; i += PAGE_SIZE){
				//insert_sorted_allocList(BLOCK);
				struct FrameInfo * ptr_frame = NULL;
				allocate_frame(&ptr_frame);
				map_frame(myenv->env_page_directory, ptr_frame, i, PERM_WRITEABLE | PERM_PRESENT | PERM_USER);
				add_frame_to_storage(shared_var->framesStorage , ptr_frame , j);
				j++;
			}
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	if(m == E_SHARED_MEM_NOT_EXISTS){
		...
		j = 0;
		uint32 end = ROUNDUP((uint32)virtual_address + shares[m].size, PAGE_SIZE);
		for(uint32 i = startaddr; i < end; i += PAGE_SIZE){
			struct FrameInfo * ptr_frame = get_frame_from_storage(shares[m].framesStorage,j);
			if(shares[m].isWritable == 1){
			map_frame(myenv->env_page_directory, ptr_frame, i, PERM_WRITEABLE | PERM_PRESENT | PERM_USER);
			}
			else{
				map_frame(myenv->env_page_directory, ptr_frame, i, PERM_PRESENT | PERM_USER);
			}
		}
		shares[m].references++;
		return m;
