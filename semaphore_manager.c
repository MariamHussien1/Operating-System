...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	struct Semaphore *m;
	int a = get_semaphore_object_ID(ownerEnvID, semaphoreName);
	if(a == E_SEMAPHORE_NOT_EXISTS){
		int z = allocate_semaphore_object(&m);
		if(z == E_NO_SEMAPHORE){
			return E_NO_SEMAPHORE;
		}
		else{
			m->ownerID = ownerEnvID;
			m->value = initialValue;
			strcpy(m->name, semaphoreName);
			return z;
		}
	}
	else{
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	semaphores[m].value -=1;
	if(semaphores[m].value < 0){
		enqueue(&(semaphores[m].env_queue),curenv);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	semaphores[m].value +=1;
	if(semaphores[m].value <= 0){
		struct Env* new_env = dequeue(&(semaphores[m].env_queue));
		sched_insert_ready(new_env);
		new_env->env_status=ENV_READY;
	}
}
